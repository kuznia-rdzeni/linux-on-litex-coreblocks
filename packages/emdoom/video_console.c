...
static int IsKBHit()
{
	...
	return !!byteswaiting;
	//unsigned long uart_empty = 0xe0007808L;
    //return !(csr_read_simple(uart_empty));
}
...
/*    
	unsigned long uart_empty = 0xe0007808L;
	unsigned long uart = 0xe0007800L;
    //while(csr_read_simple(uart_empty));
    if (!IsKBHit())
        return 0xffffffff;
    return csr_read_simple(uart) & 0xff; */
}
...
{
	unsigned long uart = 0xe0007800L;
	unsigned  long uart_full = 0xe0007804L;
    while(*s) {
        //asm volatile ("nop");

        while (csr_read_simple(uart_full));
        csr_write_simple(*s, uart);
        s++;
    }
